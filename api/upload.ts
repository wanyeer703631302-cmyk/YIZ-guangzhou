...
import { uploadImage, cloudinary } from './_lib/cloudinary'
...
const upload = multer({
  ...
  fileFilter: (_req: any, file: any, cb: any) => {
    ...
    if (!ALLOWED_MIME_TYPES.includes(file.mimetype)) {
      cb(new Error('?™ÊîØ?ÅÂõæ?áÊ†ºÂºèÊ?‰ª?(JPEG, PNG, GIF, WebP, SVG)'))
      return
    }
    cb(null, true)
  }
  ...
  withAuth(req as AuthRequest, res, async () => {
    try {
      ...
      if (!isDatabaseAvailable) {
        res.status(503).json({
          success: false,
          error: '?∞ÊçÆÂ∫ìÊ??°‰??ØÁî®ÔºåËØ∑?îÁ≥ªÁÆ°Á??òÈ?ÁΩ?DATABASE_URL'
        })
        return
      }
      ...
      try {
        ...
        res.status(500).json({
          success: false,
          error: '?æÁ?‰∏ä‰??∞‰?Â≠òÂÇ®Â§±Ë¥•ÔºåËØ∑Á®çÂ??çË?'
        })
        return
      }
      ...
      try {
        ...
        if (verifyAsset.length === 0) {
          ...
          res.status(500).json({
            success: false,
            error: '?∞ÊçÆÂ∫ìËÆ∞ÂΩïÈ?ËØÅÂ§±Ë¥•Ô?ËØ∑È?ËØ?
          })
          return
        }
        ...
        res.status(500).json({
          success: false,
          error: `‰øùÂ??æÁ?ËÆ∞Â??∞Êï∞?ÆÂ?Â§±Ë¥•: ${errorMessage}`
        })
        return
      }
    } catch (error: unknown) {
      ...
      if (error && typeof error === 'object' && 'code' in error) {
        ...
        res.status(400).json({
          success: false,
          error: `?á‰ª∂‰∏ä‰??ôËØØ: ${multerError.message}`
        })
        return
      }
